...
public class AdjacencyListGraph<E> implements GraphADT<E>
{
   ...
   {  // add all the vertices to this graph and keep track of the
      ...
      {  // obtain the one or two end vertices for the old edge
         ...
         addEdge(correspondence.get(oldEndVertices[0]),
            correspondence.get(oldEndVertices[1]), oldEdge.setstrenght());
      }
   }
   ...
   public Edge<E> addEdge(Vertex<E> vertex0, Vertex<E> vertex1, double strength)
   {  // first add the end vertices if not already in graph
      ...
      if (!containsVertex(vertex1))
         addVertex(vertex1);
      // create the new edge
      Edge<E> edge = new AdjacencyListEdge(vertex0, vertex1, strength);
      edges.add(edge);
   ...
   protected class AdjacencyListEdge implements Edge<E>
   {
      ...
      public AdjacencyListEdge(Vertex<E> vertex1, Vertex<E> vertex2, double strength)
      {  this.vertex1 = vertex1;
         this.vertex2 = vertex2;
         this.strength = strength;
         this.weight = //Math.round( -Math.log(strength) * 1000)/1000.0;
         -Math.log(strength);
      }
      ...
      public String toString()
      {  return "(" + vertex1 + "-" + vertex2 + "," + String.format("%.3f", weight) + ")"; 
      }
   }
}
